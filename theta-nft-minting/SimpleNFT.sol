...
contract TNT721 is Context, TNT165, ITNT721, ITNT721Metadata, ITNT721Enumerable {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(TNT165, ITNT165) returns (bool) {
        return interfaceId == _INTERFACE_ID_TNT721 || interfaceId == _INTERFACE_ID_TNT721_METADATA || super.supportsInterface(interfaceId);
    }
    ...
    function _checkOnTNT721Received(address from, address to, uint256 tokenId, bytes memory _data) private returns (bool) {
        if (!to.isContract()) {
            return true;
        }
        bytes memory returndata = to.functionCall(abi.encodeWithSelector(ITNT721Receiver(to).onTNT721Received.selector, _msgSender(), from, tokenId, _data), "TNT721: transfer to non TNT721Receiver implementer");
